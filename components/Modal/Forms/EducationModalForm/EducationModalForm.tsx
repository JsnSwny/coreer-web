import { addEducation, deleteEducation, updateEducation } from "@/api/educations";
...
const EducationModalForm = ({ closeModal, item }: ModalFormProps) => {
  ...
  const handleDelete = async () => {
    ...
    setUser({
      ...user!,
      educations: [...user!.educations.filter((education) => education.id != item!.id)],
    });
    closeModal();
  };
  ...
  const loadSchoolOptions = async (inputValue: string) => {
    const options = await axios.get(`${server}/api/schools/?search=${inputValue}`);
    ...
    <form onSubmit={handleSubmit(onSubmitHandler)}>
      <div className={globalStyles.modalBody}>
          ...
          <Controller
            ...
            render={({ field }) => (
              <AsyncSelect
                id="school"
                name="school"
                value={schoolOptions.filter((item: SchoolSelect) => item.value == field.value)}
                defaultOptions
        ...
        <div className={globalStyles.formGroup}>
          ...
          <input className={globalStyles.input} type="text" {...register("degree")} />
        ...
        <div className={globalStyles.formGroup}>
          ...
          <textarea className={globalStyles.input} {...register("description")} rows={4}></textarea>
