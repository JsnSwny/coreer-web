...
const ProjectModalForm = ({ closeModal, item }: ModalFormProps) => {
  ...
  const onSubmitHandler = async (data: {
    ...
    let obj: ProjectRequest = {
      ...
      image,
      start_date: data.start_date ? format(data.start_date, "yyyy-MM-dd") : data.start_date,
      end_date: data.end_date ? format(data.end_date, "yyyy-MM-dd") : data.end_date,
      user: user!.id,
    ...
    <form onSubmit={handleSubmit(onSubmitHandler)}>
      <div className={globalStyles.modalBody}>
        ...
        <div className={globalStyles.formGroup}>
          ...
          <input autoFocus className={globalStyles.input} type="text" {...register("title")} />
          <FormError message={errors.title?.message} />
        </div>
        <DateRangeInput control={control} errors={errors} watch={watch} startDateRequired={false} />
        <div className={globalStyles.formGroup}>
