...
import { faCameraRetro, faEnvelope, faPencil, faStar } from "@fortawesome/free-solid-svg-icons";
...
const ProfileBanner = ({ profile, openModal }: ProfileBannerProps) => {
  ...
  useEffect(() => {
    setImagePreviewUrl(profile.profile_photo ? profile.profile_photo : profile.image);
  }, [profile]);
    ...
    <div className={styles.container}>
      <div className={styles.left}>
        <div className={styles.content}>
          {imagePreviewUrl && <img className={styles.profilePhoto} src={imagePreviewUrl} />}
          {profile.id == user!.id && (
            <label htmlFor="photo-upload" className={styles.changePhoto}>
              <FontAwesomeIcon icon={faCameraRetro} className={styles.changePhotoImage} />
